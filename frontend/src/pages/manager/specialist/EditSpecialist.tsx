...
const EditSpecialist = () => {
  ...
  const { data: specialist, isPending: isLoading } = useFetchSpecialist(
    Number(id)
  );
  ...
  useEffect(() => {
    if (specialist) {
      ...
      setValue("price", specialist.price.toString());
      setValue("about", specialist.about);
      if (specialist.photo) {
        setImagePreview(specialist.photo);
      }
    }
  }, [specialist, setValue]);
